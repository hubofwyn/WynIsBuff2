...
function extractJSDoc(content, filePath) {
  ...
  jsdocBlocks.forEach(block => {
    const nextClass = classes.find(c => c.index > block.endIndex && c.index - block.endIndex < 100);
    const nextFunction = functions.find(f => f.index > block.endIndex && f.index - block.endIndex < 100);
