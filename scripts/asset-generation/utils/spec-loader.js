...
export class SpecLoader {
    constructor(schemaPath = null) {
        this.schemaPath =
            schemaPath || resolve(process.cwd(), 'architecture/asset-spec.schema.json');
        this.schema = null;
    ...
    validateBasicStructure(spec, specPath) {
        ...
        if (!validTypes.includes(spec.generation.type)) {
            ...
            throw new Error(
                `${specPath}: Invalid generation.type '${spec.generation.type}'. Must be one of: ${validTypes.join(', ')}`
            );
        }
        ...
        if (!validProviders.includes(spec.generation.provider)) {
            logValidationError(
                'generation.provider',
                `Invalid provider: ${spec.generation.provider}`
            );
            throw new Error(
                `${specPath}: Invalid provider '${spec.generation.provider}'. Must be one of: ${validProviders.join(', ')}`
            );
        }
