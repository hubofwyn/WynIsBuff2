...
class SimpleArchitectureValidator {
  ...
  run() {
    ...
    if (existsSync(this.specPath)) {
      ...
      console.log(`   Stack: Vite ${spec.meta.vite}, Phaser ${spec.meta.phaser}, Rapier ${spec.meta.rapier}`);
    }
  ...
  checkViolation(file, importPath, spec) {
    ...
    if ((layer === 'game-systems' || layer === 'player-agent' || layer === 'enemy-agent') &&
        (importPath === 'phaser' ||
  ...
  generateReport(snapshot) {
    ...
    // Layer distribution
    console.log('\n📐 Layer Distribution:');
    const layers = Object.entries(snapshot.stats.layerCounts)
      .sort((a, b) => b[1] - a[1]);

    for (const [layer, count] of layers) {
      const percentage = ((count / snapshot.stats.totalFiles) * 100).toFixed(1);
      console.log(`  ${layer.padEnd(20)} ${count.toString().padStart(4)} files (${percentage}%)`);
    }
