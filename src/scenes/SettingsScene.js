...
export class SettingsScene extends BaseScene {
    ...
    create() {
        ...
        this.add.text(width / 2, height / 2 - 140, 'Settings', titleStyle).setOrigin(0.5);
        ...
        volumeKeys.forEach((key, idx) => {
            ...
            // Label
            this.add
                .text(width / 2 - 200, y, volumeLabels[key] + ':', UIConfig.text.label)
                .setOrigin(0, 0.5);
            // Minus button
            const minus = this.add
                .text(width / 2 - 50, y, '<', menuButton)
                .setOrigin(0.5)
                .setInteractive();
            focusables.push(minus);
            // Percentage text
            const percText = this.add
                .text(
                    width / 2,
                    y,
        ...
        actions.forEach((action, idx) => {
            ...
            // Action label
            this.add
                .text(width / 2 - 200, y, `${actionLabels[action]}:`, UIConfig.text.label)
                .setOrigin(0, 0.5);
            // Current key text
            const current = keyBindings[action] || '';
            const keyText = this.add
                .text(width / 2, y, current, UIConfig.text.stats)
                .setOrigin(0.5)
                .setInteractive();
        ...
        // Label
        this.add
            .text(width / 2 - 200, gfxY, 'Graphics Quality:', UIConfig.text.label)
            .setOrigin(0, 0.5);
        // Left arrow
        const leftArrow = this.add
            .text(width / 2 - 50, gfxY, '<', UIConfig.menuButton)
            .setOrigin(0.5)
            .setInteractive();
        focusables.push(leftArrow);
        // Quality text
        const qualityText = this.add
            .text(width / 2, gfxY, qualityOptions[qualityIndex], UIConfig.text.stats)
            .setOrigin(0.5);
        // Right arrow
        ...
        const updateQuality = (delta) => {
            ...
            gameState.saveSettings(
                Object.assign({}, settings, { graphicsQuality: currentQuality })
            );
        };
        // Interactions
        ...
        // Label
        this.add
            .text(width / 2 - 200, accStartY, 'Color-blind Palette:', UIConfig.text.label)
            .setOrigin(0, 0.5);
        // Left arrow
        const palLeft = this.add
            .text(width / 2 - 50, accStartY, '<', UIConfig.menuButton)
            .setOrigin(0.5)
            .setInteractive();
        focusables.push(palLeft);
        // Palette text
        const palText = this.add
            .text(width / 2, accStartY, paletteOptions[paletteIndex], UIConfig.text.stats)
            .setOrigin(0.5);
        // Right arrow
        const palRight = this.add
            .text(width / 2 + 50, accStartY, '>', UIConfig.menuButton)
            .setOrigin(0.5)
            .setInteractive();
        ...
        const updatePalette = (delta) => {
            ...
            gameState.saveSettings(
                Object.assign({}, settings, {
                    accessibility: Object.assign({}, acc, { palette: newPal }),
                })
            );
        };
        ...
        this.add
            .text(width / 2 - 200, hcY, 'High Contrast:', UIConfig.text.label)
            .setOrigin(0, 0.5);
        const hcText = this.add
            .text(width / 2, hcY, acc.highContrast ? 'On' : 'Off', UIConfig.text.stats)
            .setOrigin(0.5)
            .setInteractive();
        focusables.push(hcText);
        hcText.on('pointerover', () => {
            hcText.setTint(UIConfig.menuButton.hoverTint);
            AudioManager.getInstance().playSFX('hover');
        });
        ...
        const subText = this.add
            .text(width / 2, subY, acc.subtitles ? 'On' : 'Off', UIConfig.text.stats)
            .setOrigin(0.5)
            .setInteractive();
        focusables.push(subText);
        subText.on('pointerover', () => {
            subText.setTint(UIConfig.menuButton.hoverTint);
            AudioManager.getInstance().playSFX('hover');
        });
        ...
        const backBtn = this.add
            .text(width / 2, height / 2 + 200, 'Back', menuButton)
            .setOrigin(0.5)
            .setInteractive();
