...
export class FeedbackSystem extends BaseManager {
    ...
    setupEventListeners() {
        ...
        eventBus.on(
            EventNames.PERFORMANCE_METRIC_RECORDED,
    ...
    createFeedbackContainers() {
        ...
        this.floatingContainer = this.scene.add.container(0, 0).setDepth(9000).setScrollFactor(0);
        ...
        this.progressContainer = this.scene.add.container(0, 0).setDepth(8000).setScrollFactor(0);

        // Screen effects container (full screen overlay)
        this.effectsContainer = this.scene.add.container(0, 0).setDepth(9500).setScrollFactor(0);
    }
    ...
    showNotification(title, message, type = 'neutral', options = {}) {
        ...
        const bg = this.scene.add
            .rectangle(0, 0, 350, 70, 0x000000, 0.8)
    ...
    showProgressBar(id, current, max, options = {}) {
        ...
        if (!progressBar) {
            ...
            const bg = this.scene.add
                .rectangle(0, 0, width, height, 0x333333)
                .setStrokeStyle(2, 0x666666);

            const fill = this.scene.add
                .rectangle(-width / 2, 0, 0, height - 4, 0x44ff44)
                .setOrigin(0, 0.5);
    ...
    handleCloneForged(data) {
        this.showNotification(
            'Clone Forged!',
            `New clone lane created with ${data.rate}/s production`,
            'success'
        );
    }

    handleMovementUnlocked(data) {
        this.showNotification('Movement Unlocked!', `New ability: ${data.tech}`, 'combo');
    }
