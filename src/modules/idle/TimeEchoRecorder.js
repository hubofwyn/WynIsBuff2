...
export class TimeEchoRecorder {
    ...
    detectPatterns(inputs) {
        ...
        // Wall jump sequence
        if (this.matchesSequence(inputs, ['jump', 'move_left', 'jump']) ||
    ...
    matchesSequence(inputs, sequence) {
        ...
        return sequence.every((action, i) => 
            recent[i].inputType === action || 
            (action === 'move' && (recent[i].inputType === 'move_left' || recent[i].inputType === 'move_right'))
        );
    }
    ...
    compressRecording() {
        ...
        this.currentRecording.timeline = this.currentRecording.timeline.filter(
            event => essentialTypes.includes(event.type) || 
    ...
    processPlayback(cloneId) {
        ...
        while (playback.currentIndex < echo.timeline.length) {
            ...
            if (event.timestamp <= elapsed) {
                ...
                const processedEvent = playback.mutate ? 
                    this.mutateEvent(event) : event;
    ...
    reinforcePattern(data) {
        ...
        if (patterns && patterns.length > 0) {
            ...
            if (recent) {
                recent.effectiveness = Math.min(1.0, 
    ...
    weakenPattern(data) {
        ...
        if (patterns && patterns.length > 0) {
            ...
            const recent = patterns[patterns.length - 1];
            if (recent) {
                recent.effectiveness = Math.max(0.0, 
