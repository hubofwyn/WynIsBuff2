...
export class EnhancedMovementController {
    ...
    calculateGroundMovement(currentVelX) {
        ...
        if (this.currentInput.direction !== 0) {
            ...
            const changingDirection = (currentVelX > 0 && this.currentInput.direction < 0) ||
    ...
    calculateAirMovement(currentVelX) {
        ...
        if (this.currentInput.direction !== 0) {
            ...
            if (changingDirection) {
                ...
                return currentVelX + (targetSpeed - currentVelX) * params.acceleration * params.turnAroundBoost;
            } else {
        ...
        switch (this.state) {
            ...
            case 'airborne':
                if (isOnGround) {
                    ...
                    if (vel.y > 0) { // Only slide when falling
    ...
    interpolateVisuals(sprite, body, alpha) {
        ...
        const interpolatedX = this.previousPosition.x + (currentPos.x - this.previousPosition.x) * alpha;
        const interpolatedY = this.previousPosition.y + (currentPos.y - this.previousPosition.y) * alpha;
