...
export class CloneManager extends BaseManager {
    ...
    calculateCombatStat(runData) {
        const baseCombat = 5;
        const combatScore =
            (runData.enemiesDefeated / runData.totalEnemies) *
    ...
    calculateHeightStat(runData) {
        const baseHeight = 5;
        const jumpScore =
            (runData.maxJumpHeight / runData.averageJumpHeight) *
    ...
    calculateResourceStat(runData) {
        ...
        const collectScore =
            (runData.collectiblesGathered / runData.totalCollectibles) *
    ...
    calculateBuffStat(runData) {
        const baseBuff = 5;
        const buffScore =
            (runData.powerUpsUsed / runData.powerUpsCollected) *
    ...
    checkCloneLevelUp(clone) {
        ...
        if (xpGained >= xpNeeded) {
            ...
            for (const stat in clone.dna.stats) {
                clone.dna.stats[stat] = Math.min(this.statCaps[stat], clone.dna.stats[stat] * 1.02);
            }
