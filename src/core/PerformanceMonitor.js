...
export class PerformanceMonitor extends BaseManager {
    ...
    collectMetrics() {
        ...
        if (
            !this.metricsCollection.enabled ||
    ...
    handleRunComplete(data) {
        ...
        if (data.runTime) {
            ...
            const totalTime =
                this.gameMetrics.averageRunTime * (this.gameMetrics.runsCompleted - 1) +
                data.runTime;
        ...
        if (data.runScore) {
            ...
            if (
                newScore.S + newScore.C + newScore.H + newScore.R + newScore.B >
            ...
            averages: {
                ...
                deltaTime:
                    this.deltaHistory.length > 0
                        ? this.deltaHistory.reduce((sum, delta) => sum + delta, 0) /
                          this.deltaHistory.length
                        : 0,
            },
            peaks: {
                ...
                worstDelta:
                    this.deltaHistory.length > 0 ? Math.max(...this.deltaHistory) : this.deltaTime,
            },
            gameMetrics: { ...this.gameMetrics },
