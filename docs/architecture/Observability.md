...
export const ErrorCodes = Object.keys(ErrorCodeRegistry).reduce(
    (acc, key) => {
        acc[key] = key;
        return acc;
    },
    ...
    static assertRange(
        ...
        this.assert(value >= min && value <= max, {
            ...
            data: {
                ...
                min,
                max,
                outOfBounds: value < min ? 'below' : 'above',
            },
        });
    }
...
export class DebugAPI {
    ...
    getState(target: string): any {
        switch (target.toLowerCase()) {
            case 'player':
                return (
                    this.game?.playerController?.getDebugSnapshot?.() || {
            ...
            case 'physics':
                return (
                    this.game?.physicsManager?.getDebugSnapshot?.() || {
            ...
            case 'input':
                return (
                    this.game?.inputManager?.getDebugSnapshot?.() || {
                        error: 'Input manager not available',
                    }
                );

            case 'context':
                return this.context?.toJSON() || { error: 'Debug context not bound' };
            ...
            default:
                return {
                    error: `Unknown target: ${target}. Try: player, physics, input, context, all`,
                };
        }
    }
...
export class OTelAdapter {
     ...
     */
    wrapInSpan<T>(spanName: string, fn: (span: Span) => T, attributes?: Record<string, any>): T {
        if (!this.enabled) {
...
const mostFrequent = Object.entries(errorCounts).sort((a, b) => b[1] - a[1])[0];
