    ...
    rules: {
      ...
      'boundaries/element-types': ['warn', {
        ...
        rules: [
          ...
          {
            from: 'game-systems',
            allow: ['core', 'player-agent', 'enemy-agent', 'constants', 'observability']
          },
...
class BuildBenchmark {
  ...
  analyzeResults() {
    ...
    const improvement = (
      ...
      this.results.rollup.average * 100
    ).toFixed(2);

    this.results.improvement = improvement;
    this.results.recommendation = improvement > 30
      ? 'ADOPT'
      : improvement > 10
        ? 'CONSIDER'
        : 'SKIP';

    writeFileSync('./docs/architecture/build-benchmark.json',
...
export class ReplaySystem {
  ...
  exportReplay() {
    const ndjson = this.frames.map(frame =>
